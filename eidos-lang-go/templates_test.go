...
func TestTemplates(t *testing.T) {
	...
	t.Run("struct", func(t *testing.T) {
		...
		assert.Equal(t, execute(t, golang.StructTemplate, s),
			"// Row is one record.\n"+
				"type Row struct {\n"+
	...
	t.Run("interface", func(t *testing.T) {
		...
		assert.Equal(t, execute(t, golang.InterfaceTemplate, i),
			"// Store loads rows.\n"+
	...
	t.Run("function and method place the body", func(t *testing.T) {
		...
		assert.Equal(t, execute(t, golang.FunctionTemplate, f),
		...
		assert.Equal(t, execute(t, golang.MethodTemplate, m),
			"func (s *Store) Close() {\n\tbody()\n}\n", "the method's shape")
	})
	...
	t.Run("alias, constant and variable", func(t *testing.T) {
		...
		assert.Equal(t,
			execute(t, golang.AliasTemplate, &emit.Alias{Name: "ID", Target: ref("string")}),
			"type ID = string\n", "the alias shape")
		assert.Equal(t,
			execute(t, golang.ConstantTemplate,
				&emit.Constant{Name: "Max", Type: ref("int"), Value: "10"}),
			"const Max int = 10\n", "a typed constant")
		assert.Equal(t,
			execute(t, golang.ConstantTemplate, &emit.Constant{Name: "Max", Value: "10"}),
			"const Max = 10\n", "an untyped one")
		assert.Equal(t,
			execute(t, golang.VariableTemplate,
	...
	t.Run("the file skeleton", func(t *testing.T) {
		...
		got := execute(t, golang.FileTemplate, struct {
			Name string
			Pkg  symbol.Identity
