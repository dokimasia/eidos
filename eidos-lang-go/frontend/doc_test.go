...
// hermeticity that position promises is held over the whole import
// graph.
func TestDoc(t *testing.T) {
	...
	t.Run("never imports os/exec", func(t *testing.T) {
		...
		assert.True(t, slices.Contains(deps, "go/parser"),
			"the listing reaches the parser the frontend is built on")
