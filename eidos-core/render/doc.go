...
// Package render is the pass every backend runs: the ritual the
// languages share, owned once.
//
// [New] composes a [Language], the handful of things a target
// genuinely varies in, into a [Pass]. [Pass.Render] takes one
// plan's emit store through the ritual: group units into files
...
// each file through the language formatter, and answer files as
// values. Nothing lands on disk: staging, headers and trailers
...
// # Dependency position
//
// core/render imports core/plugin, core/diag, core/position,
// core/symbol and the Go stdlib, text/template included. It never
// imports the root authoring package: the backend kit there lowers
// to this pass, so the pass works at the floor.
package render
