...
// valueless is a fixture renderer answering files as values.
...
// composition's hands: a renderer answers values, and the provider
// surfaces declare the trees references resolve in.
func TestRenderer(t *testing.T) {
	t.Parallel()

	t.Run("a renderer answers files as values", func(t *testing.T) {
		t.Parallel()
		...
		assert.Length(t, files, 1, "answering its files")
	...
	t.Run("a template provider declares per target", func(t *testing.T) {
		...
		assert.True(t, declared, "the declared target answers its tree")
		_, err := fs.Stat(tree, "method1.tpl")
		assert.NoError(t, err, "holding the referenced template")
		_, declared = tp.Templates("elsewhere")
		assert.False(t, declared, "an undeclared target answers nothing")
	})
}
