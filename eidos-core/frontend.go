...
func NewFrontend(
	...
	return &FrontendBuilder{name: name, lang: lang, syntax: syntax}
}
...
// reports why.
func (b *FrontendBuilder) Version(v string) *FrontendBuilder {
	b.version = v
	return b
}
...
func (b *FrontendBuilder) Match(patterns ...string) *FrontendBuilder {
...
// order.
func (b *FrontendBuilder) Classify(cs ...Classifier) *FrontendBuilder {
...
func (b *FrontendBuilder) Resolve(
	resolve func(plugin.ImportScope, string) []symbol.Identity,
) *FrontendBuilder {
	b.resolve = resolve
	return b
...
func (b *FrontendBuilder) Options(o any) *FrontendBuilder {
	b.options = o
	b.hasOptions = true
	return b
}
...
func (b *FrontendBuilder) Build() plugin.Frontend {
	if b.name == "" {
		panic("eidos: NewFrontend with an empty name")
	}
	name := string(b.name)
	var defects []string
	if b.lang == "" {
	...
	if len(defects) > 0 {
		panic("eidos: " + name + " " + strings.Join(defects, ", and "))
	}
	base := &builtFrontend{
