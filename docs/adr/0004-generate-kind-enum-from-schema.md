...
The constants have to match the schema structs one to one, because
every generated `Kind()` method returns one and every consumer
switches on them.
...
unpinned `go run ...@version` fetch at generate time.
...
- The generator becomes load-bearing for the kernel's most central
  type. The mirror guard catches drift, but a generator bug now
  breaks more than the models.
