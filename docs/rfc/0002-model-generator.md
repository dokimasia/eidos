---
rfc: 0002
title: The model generator (internal/gen/symbol)
...
# RFC-0002: The model generator (internal/gen/symbol)

## Summary

`internal/gen/symbol` turns the `symbol/schema` package of RFC-0001 into the
...
The tool lives at `internal/gen/symbol`, named for the package it
reads. Two more generators take the same input or the same shape: the
...
Files group by concern rather than by kind (ADR-0005): eight files
instead of thirty-four, diffs review as one concern at a time, and the
`.gen.go` suffix rides the license-header exclusion already in
`.ergon.yaml`. Every file opens with
`// Code generated by internal/gen/symbol. DO NOT EDIT.`, the standard
...
```go
// internal/gen/symbol exposes one function; main is a thin wrapper.
// Generate renders every output file into memory.
func Generate(modRoot string) (map[string][]byte, error)

// internal/gen/symbol/mirror_test.go — the mirror guard.
